function standardHoursController($scope, angularHelper, localizationService) {
  ...
  $scope.model.config.showBankHolidays = Object.toBoolean(
    $scope.model.config.showBankHolidays
  );
  ...
  localizationService
    ...
    .then(function (data) {
      ...
      if (
        !$scope.model.config.labelBankHolidays ||
        $scope.model.config.labelBankHolidays.length === 0
      ) {
  ...
  $scope.deleteHours = function (index, parentIndex) {
    ...
    if (
      !Array.isArray(day.hoursOfBusiness) ||
      day.hoursOfBusiness.length === 0
    ) {
  ...
  function init() {
    ...
    if (!$scope.model.value) {
      ...
      if (
        !$scope.model.config.showBankHolidays &&
        $scope.model.value.length === 8
      ) {
        $scope.model.value.pop();
      } else if (
        $scope.model.config.showBankHolidays &&
        $scope.model.value.length === 7
      ) {
    ...
    if ($scope.model.config.showBankHolidays) {
      $scope.model.value[7].dayoftheweek =
        $scope.model.config.labelBankHolidays;
    }
...
angular
  .module('umbraco')
  .controller(
    'OpenOrClosed.PropertyEditors.StandardHours.controller',
    standardHoursController
  );
